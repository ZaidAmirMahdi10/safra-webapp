...
import { Container, Typography, List, ListItem, ListItemText, Collapse, TextField, Checkbox, FormControlLabel } from '@mui/material';
...
const Dashboard = () => {
    ...
    return (
      ...
      users[type].filter((user) => {
        ...
        return (
          ...
          (type === 'company' && name.toLowerCase().includes(searchTerm.toLowerCase()))
        );
      })
    );
  };
  ...
  const handleSelectCompany = (companyId) => {
    setSelectedCompany((prevSelected) => (prevSelected === companyId ? null : companyId));
  };
  ...
  return (
    <Container maxWidth="md">
      <div>
        ...
        {/* Products */}
        <Typography variant="h5" align="left" gutterBottom onClick={() => handleToggle('products')}>
          ...
          <List>
            {products.map((product) => (
              <ListItem key={product.id}>
                <ListItemText primary={product.name} secondary={product.company?.name || ''} />
              </ListItem>
            ))}
          </List>
          <br/>
        ...
        <Typography variant="h5" align="left" gutterBottom onClick={() => handleToggle('users')}>
          {openUsers ? '▼' : '▶'} Users
        </Typography>

        <Collapse in={openUsers}>
          {/* Customers */}
          <Typography variant="h5" align="left" gutterBottom onClick={() => handleToggle('customer')}>
          ...
          {/* Companies */}
          <Typography variant="h5" align="left" gutterBottom onClick={() => handleToggle('company')}>
          ...
          {/* Admins */}
          <Typography variant="h5" align="left" gutterBottom onClick={() => handleToggle('admin')}>
            {openAdmins ? '▼' : '▶'} Admins
          </Typography>
